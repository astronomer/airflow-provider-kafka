...
class ProduceToTopic(BaseOperator):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        if not (self.topic and self.producer_function):
            raise AirflowException(f"topic and producer_function must be provided. Got topic={self.topic} and producer_function={self.producer_function}")
    ...
    def execute(self,context) -> Any:
        ...
        producer = ProducerHook(kafka_conn_id=self.kafka_conn_id, config = self.kafka_config, no_broker=self.no_broker).get_producer()
        producer_callable = get_callable(self.producer_function)
        producer_callable = partial(producer_callable, *self.producer_function_args, **self.producer_function_kwargs)
